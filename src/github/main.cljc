(ns github.main
  "Kotodama WASM entrypoint for the Github clean-room actor (L4) — Clojure port.
  ...
  facts (`github.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "github")
(def tier "L4")
...
(def entity-specs
  [{:entity "Repository" :plural "repositories" :id-prefix "github_rep"
    :fields [:name :owner :defaultBranch :private :stars]          :required [:name :owner]
    :coerce {:private :bool :stars :int}     :refs {}}
   {:entity "Issue"      :plural "issues"      :id-prefix "github_iss"
    :fields [:repoId :number :title :state :authorId]             :required [:number :title]
    :coerce {:number :int}                    :refs {:repoId "Repository"}}
   {:entity "PullRequest" :plural "pullrequests" :id-prefix "github_pul"
    :fields [:repoId :number :title :state :headRef :baseRef]     :required [:number :title]
    :coerce {:number :int}                    :refs {:repoId "Repository"}}
   {:entity "Commit"    :plural "commits"    :id-prefix "github_com"
    :fields [:repoId :sha :message :authorId]                     :required [:sha :message]
    :coerce {}                                :refs {:repoId "Repository"}}
   {:entity "Workflow"  :plural "workflows"  :id-prefix "github_wor"
    :fields [:repoId :name :state]                                :required [:name :state]
    :coerce {}                                :refs {:repoId "Repository"}}
   {:entity "WorkflowRun" :plural "workflowruns" :id-prefix "github_wor"
    :fields [:workflowId :status :conclusion :commitSha]          :required [:status :conclusion]
    :coerce {}                                :refs {:workflowId "Workflow"}}
   {:entity "Release"   :plural "releases"   :id-prefix "github_rel"
    :fields [:repoId :tag :name :draft]                           :required [:tag :name]
    :coerce {:draft :bool}                    :refs {:repoId "Repository"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"github.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "github-compat" :tier tier :entities entities} 200])
...
(defn start! [] :github-compat/ready)
